...
/* -----------------------
   Auto-scroll carousel (seamless loop)
----------------------- */
function useAutoScrollLoop(ref, speed = 0.6) {
...
export default function Events() {
  ...
  const [selectedISO, setSelectedISO] = useState(() => events[0]?.date)
  ...
  useAutoScrollLoop(carouselRef, 0.6)
  const highlightsLoop = useMemo(() => [...highlights, ...highlights], [highlights])
  ...
  return (
    <section id="events" className="section">
      ...
      <div className="carouselWrap">
        ...
        <div className="carousel" ref={carouselRef} aria-label="Highlights carousel">
          {highlightsLoop.map((h, idx) => {
            const mapped = highlightMap?.[h.key]
            const src = mapped || h.image || ''
            return (
              <div className="slide" key={`${h.key}-${idx}`}>
                ...
                <div className="slideText">
                  <div className="slideStamp">
                    <span className="stampPill">{h.week}</span>
                    <span className="stampDot" aria-hidden="true">•</span>
                    <span className="stampMonth">{h.month}</span>
                  </div>
                  <div className="slideTitle">{h.title}</div>
                  <div className="muted">{h.sub}</div>
                </div>
              </div>
            )
          })}
