...
export default function Events() {
  ...
  const { copies: highlightCopies } = useSeamlessCarousel({
    viewportRef: carouselViewportRef,
    trackRef: carouselTrackRef,
    firstSetRef: carouselGroupRef,
    secondsPerLoop: 16,
    pauseOnHover: true,
    enableWheel: true,
    enableKeyboard: true,
  })
  const highlightsLoop = highlights
  ...
  return (
    <section id="events" className="section">
      ...
      <div className="carouselWrap">
        ...
        <div className="carouselViewport" ref={carouselViewportRef} aria-label="Highlights carousel" tabIndex={0}>
          <div className="carouselTrack" ref={carouselTrackRef}>
            {Array.from({ length: highlightCopies }).map((_, copyIndex) => (
            <div
              className="carouselGroup"
              ref={copyIndex === 0 ? carouselGroupRef : null}
              aria-hidden={copyIndex > 0}
              key={`hl-group-${copyIndex}`}
            >
              {highlights.map((h) => {
                const mapped = highlightMap?.[h.key]
                const src = mapped || h.image || ''
                return (
                  <div className="slide" key={`${copyIndex}-${h.key}`}>
                    ...
                    <div className="slideText">
                      ...
                      <div className="slideSub muted">{h.desc}</div>
