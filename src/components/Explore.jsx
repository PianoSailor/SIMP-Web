...
export default function Explore({ lang = 'en', showInstagramEmbed = false }) {
  ...
  const manualEmbedUrl = data?.instagramEmbedUrl?.trim?.() || ''
  const instagramUsername = data?.instagramUsername?.trim?.() || 'unimelb.simp'

  useEffect(() => {
    let cancelled = false
    async function run() {
      // If a manual post URL is provided in CMS, use it.
      if (manualEmbedUrl) {
      ...
      try {
        ...
        // If API isn't configured, we simply won't show the embed.
  ...
  const title = site?.explore?.titleitle || FALLBACK_SITE.explore.title
      ...
      <div className="sectionHead">
        <div>
          ...
          <p className="muted">{subtitle}</p>
            ...
            {showInstagramEmbed && resolvedEmbedUrl ? (
        <InstagramEmbed url={resolvedEmbedUrl} />
      ) : null}
